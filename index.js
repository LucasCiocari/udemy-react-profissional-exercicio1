    ...
    render() {
        return (
            <div className="container">
                ...
                <NoteList notes={this.state.notes} onMove={this.handleMove} onDelete={this.handleDelete} onEdit={this.handleEdit} />
            </div>
        );
    }
}
...
class Note extends React.Component {
    ...
    render() {
        ...
        return (<div className="note">
            {isEditing ? (
                <input type="text" className="note__input" defaultValue={note.text} ref={c => { this.input = c; }} />
            ) : (
            ...
            <button
                className={classNames("note__button", { "note__button--hidden": index === total - 1 })}
                onClick={() => {
                    onMove("down", index);
                }}
            >
...
const NoteList = ({ notes, onMove, onDelete, onEdit }) => (
    <div className="note-list">
        {notes.map((note, index) => (
            <Note key={note.id} note={note} onEdit={onEdit} onDelete={onDelete} index={index} onMove={onMove} total={notes.length} />
        ))}
    </div>
);
